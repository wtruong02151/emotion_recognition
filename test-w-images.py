...
with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        for j in range(BATCHES):
            ...
            t,prediction, output, error = sess.run([train_step,pred, y, err], feed_dict={x: train_data[j*BATCHSZ:(j+1)*BATCHSZ], y_: train_labels[j*BATCHSZ:(j+1)*BATCHSZ], keep_prob : .5})
            # print len(output)
            # print prediction
