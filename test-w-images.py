...
W_fc1 = weight_variable([7 * 7 * 64, 4])
...
W_fc2 = weight_variable([4, 2]) # 2nd param is number of labels we have
b_fc2 = bias_variable([2]) # 2nd param is number of labels we have
...
pred = tf.equal(tf.argmax(y_conv,1), tf.argmax(y_,1))
